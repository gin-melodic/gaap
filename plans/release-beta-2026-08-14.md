...
- **已完成（上午提前完成）**：部署 `gaap.cc`，执行公网注册、登录、账户、交易、
  Dashboard、refresh、logout、HTTPS 和容器重启 smoke，并开放邀请制邮箱白名单；
- **已完成**：12:15–14:20 CST 执行发布后观察。13:20 和 14:20 检查时公网 live/ready
  均为 200，5 个 GAAP 生产容器均为 `healthy`；RabbitMQ 两队列各 1 个消费者且无积压；
  PostgreSQL 无锁等待或超过 5 分钟的事务；运行中服务日志无 ERROR/WARN/PANIC/FATAL、
  HTTP 5xx、ALE/refresh 或 Redis 失败模式；最终只读对账仍为 6 个账户、4 笔交易，
  `passed=true`、无差异或完整性问题。
...
无可验证生产恢复点，可能无法恢复最新业务数据。12:15–14:20 CST 的发布后两小时观察已
完成且无异常，邀请制 Beta 阶段发布工作收口。
