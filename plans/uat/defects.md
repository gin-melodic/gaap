...
| DEF-009 | P0 | 交易事务 | 创建交易失败路径可能未回滚，提交错误被忽略 | IMPLEMENTED / INTEGRATION REQUIRED | TC-EDGE-DATA-001 | release 分支待提交 |
...
| DEF-016 | P0 | Dashboard/RabbitMQ | 生产模式跳过 RabbitMQ 初始化，交易后仅清 Redis 快照又从 PostgreSQL 恢复旧快照，历史日期交易不进入趋势图 | IMPLEMENTED / DOCKER VERIFIED / UAT REQUIRED | 历史日期交易创建、更新、删除后的趋势图；RabbitMQ/ready | release 分支待提交 |
| DEF-017 | P0 | 启动/余额 | 启动余额重算用字符串查询整数枚举且会跳过失败账户；规则不完整，不适合作为生产自动修复器 | MITIGATED / RECONCILIATION TEST REQUIRED | 容器重启余额不变；只读账户余额与交易流水对账 | release 分支待提交 |
